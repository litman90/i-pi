...
__DRIVER_NAME__ = "lammps_ghost_v2"
__DRIVER_CLASS__ = "lammps_ghost_v2_driver"
...
"""
class lammps_ghost_v2_driver(lammps_driver):

    def __init__(self, args=None, verbose=False):
        super(lammps_ghost_v2_driver, self).__init__(args, verbose=verbose, error_msg=ERROR_MSG)

    def initialize_lammps(self):
        super(lammps_ghost_v2_driver, self).initialize_lammps()
        if(np.absolute(self._displacement) >10**-6):
         self.natoms3_real  = 171*3# self.natoms3//2 #ALBERTO
         self.natoms3_ghost = 34*3#self.natoms3//2 # ALBERTO
         self.natoms_real  = 171 #self.natoms//2 #ALBERTO
         self.natoms_ghost = 34 # self.natoms//2 #ALBERTO
        else:
    ...
    def send_info(self, cell, pos):
        ...
        #Ghost system
        i0=120+17 #index0 # ALBERTO
        for i in range(self.natoms_ghost):
         for ii in range(3):
            x[self.natoms3_real+3*i+ii] = p[3*i0+3*i+ii] + self.disp_v[ii]  #ALBERTO
